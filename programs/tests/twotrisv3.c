...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniform3f(vertOffsetAddress, sin(elapsed) * 0.4f, cos(elapsed) * 0.4f, 0.0f); // left triangle
		glDrawArrays(GL_TRIANGLES, 0, 3);
		glUniform3f(vertOffsetAddress, sin(elapsed) * -0.4f, cos(elapsed) * -0.4f, 0.0f); // right triangle
		glDrawArrays(GL_TRIANGLES, 0, 3);
